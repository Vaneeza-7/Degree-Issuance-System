...
public partial class fypManageRequests : System.Web.UI.Page
{
    ...
    protected void btnApprove_Click(object sender, EventArgs e)
    {
        ...
        if (ApproveRequestFYP(token, comment))
        {
            dbHandler.sendNotification(studentID, "Your request has been approved by FYP Department.");
    ...
    protected void btnDisapprove_Click(object sender, EventArgs e)
    {
        ...
        if (RejectRequestFYP(token, comment))
        {
            dbHandler.sendNotification(studentID, "Your request has been rejected by FYP Department.");
            // Display success message
    ...
    protected bool ApproveRequestFYP(string token, string comments)
    {
        try
        {
            string connectionString = "Data Source=DESKTOP-D3DRLR7\\SQLEXPRESS;Initial Catalog=onestop;Integrated Security=True;TrustServerCertificate=True;";
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string query = "UPDATE DegreeRequests SET FYPApproved = 1, FYPComments = @comments WHERE Token = @token";
    ...
    protected bool RejectRequestFYP(string token, string comments)
    {
        try
        {
            string connectionString = "Data Source=DESKTOP-D3DRLR7\\SQLEXPRESS;Initial Catalog=onestop;Integrated Security=True;TrustServerCertificate=True;";
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string query = "UPDATE DegreeRequests SET FYPApproved = 0, FYPComments = @comments WHERE Token = @token";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@token", token);
                    cmd.Parameters.AddWithValue("@comments", comments);
